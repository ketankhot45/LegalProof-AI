      ...
      <div className="p-4 sm:p-5 border-b border-zinc-800 bg-zinc-950/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {canUpload && (
          <Link 
            ...
            className="inline-flex items-center justify-center text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white px-3.5 py-2 min-h-[40px] rounded-lg transition-colors shadow-sm whitespace-nowrap self-start sm:self-auto"
          >
        ...
        {loading ? (
          ...
          <div className="p-8 text-center flex flex-col items-center">
            ...
            {canUpload && (
              <Link 
                to={`/cases/${caseId}/evidence/upload`}
                className="mt-4 inline-flex items-center px-3.5 py-2 min-h-[44px] bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-semibold transition-colors shadow-sm"
              >
                <Upload className="w-3.5 h-3.5 mr-1.5" />
                Upload Evidence
              </Link>
          ...
          evidence.map((item) => (
            <div key={item.id} className="p-4 sm:p-4.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-zinc-800/30 transition-colors">
              <div className="flex items-start min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1 space-y-1.5">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                    ...
                    {item.sha256Hash && (
                      <HashDisplay hash={item.sha256Hash} truncate="short" size="xs" variant="inline" />
                    )}
                  </div>
                  <div className="text-xs text-zinc-400 flex flex-wrap items-center gap-x-3 gap-y-1">
                    ...
                    {item.blockchainStatus && (
                      <>
                        <span>•</span>
                        <StatusBadge type="blockchain" status={item.blockchainStatus} size="sm" />
                      </>
                    )}
                  </div>
                </div>
              ...
              <div className="sm:shrink-0 self-end sm:self-auto">
                <Link 
                  to={`/evidence/${item.id}`} 
                  className="inline-flex items-center text-xs font-semibold text-zinc-200 hover:text-white bg-zinc-800 hover:bg-zinc-700 px-3 py-2 min-h-[40px] rounded-lg border border-zinc-700 transition-colors"
                >
