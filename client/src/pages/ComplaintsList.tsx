...
export const ComplaintsList = () => {
  ...
  const getActionDetails = (complaint: any) => {
    if (user?.role === 'COMPLAINANT') {
      return { 
        label: 'View Details', 
        style: 'bg-zinc-800 hover:bg-zinc-700 text-zinc-200 border-zinc-700/80 font-medium' 
      };
    }
    switch (complaint.status) {
      case 'SUBMITTED':
        return { 
          label: 'Review & Triage', 
          style: 'bg-indigo-600 hover:bg-indigo-500 text-white font-semibold border-indigo-500 shadow-sm' 
        };
      case 'UNDER_REVIEW':
        return { 
          label: 'Review & Escalate', 
          style: 'bg-amber-600/20 hover:bg-amber-600/30 text-amber-300 border-amber-500/40 font-semibold' 
        };
      case 'ESCALATED':
        return { 
          label: 'View Record', 
          style: 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border-zinc-700/80 font-medium' 
        };
      case 'REJECTED':
        return { 
          label: 'View Record', 
          style: 'bg-zinc-800/80 hover:bg-zinc-700 text-zinc-400 border-zinc-700/60 font-medium' 
        };
      default:
        return { 
          label: 'View', 
          style: 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border-zinc-700 font-medium' 
        };
    }
  };
          ...
          <div className="p-12 text-center flex flex-col items-center">
            ...
            <p className="text-sm text-zinc-400 max-w-sm mb-4">
              {user?.role === 'COMPLAINANT' 
                ...
                : 'There are currently no complaints in the system requiring attention.'}
            </p>
            {user?.role === 'COMPLAINANT' && (
              <Link 
                to="/complaints/new" 
